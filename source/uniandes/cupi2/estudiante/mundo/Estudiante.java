...
public class Estudiante
{
    ...
    {
    	StringBuilder resumen = new StringBuilder("📋 Estado Académico por Departamento:\n\n");
        Curso[] cursos = {curso1, curso2, curso3, curso4};
        Map<Departamento, List<Double>> notasPorDepto = new HashMap<>();

        // Agrupar notas por departamento
        for (Curso curso : cursos) {
            if (curso != null && curso.estaCalificado()) {
                Departamento depto = curso.darDepartamento();
                notasPorDepto.putIfAbsent(depto, new ArrayList<>());
                notasPorDepto.get(depto).add(curso.darNota());
            }
        }

        // Generar resumen
        for (Map.Entry<Departamento, List<Double>> entry : notasPorDepto.entrySet()) {
            resumen.append(entry.getKey()).append("\n");
            
            for (double nota : entry.getValue()) {
                double diferencia = Math.max(0, NOTA_PRUEBA_ACADEMICA - nota);
                resumen.append(String.format("  Nota: %.2f \n  Te falta: %.2f\n", nota, diferencia));
            }
            resumen.append("\n");
        }

        return resumen.isEmpty() ? "No hay notas asignadas." : resumen.toString();
    }
    /**
